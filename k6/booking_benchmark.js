...
const responseTimeTrend = new Trend('booking_response_time');

export const options = {
  setupTimeout: '3m', // Allow up to 3 minutes for setup (important for slower Docker Desktop/WSL2 runtimes)
  scenarios: {
    booking_rush: {
  ...
  // ── SLO THRESHOLDS ──────────────────────────────────────────────────────────
  // Benchmark fails (exit code 1) if any of these are breached.
  // Tighten p99 values as the system improves — current targets reflect
  // post-Redis-cache + HC5 connection-pool optimisations.
  thresholds: {

    // Booking end-to-end response time (measured per VU from POST /bookings send
    // to response received, including event validation + seat lock + payment).
    // p99 < 30s  →  even the slowest 1% of bookings must finish before the
    //               Feign read-timeout fires (25s) plus 5s grace.
    'booking_response_time': [
      { threshold: 'p(99)<30000', abortOnFail: false },
    ],

    // Raw HTTP request duration across ALL requests (setup + booking).
    // p99 < 30s  →  consistent with the booking SLO above.
    'http_req_duration': [
      { threshold: 'p(99)<30000', abortOnFail: false },
    ],

    // 'checks' is k6's built-in Rate metric tracking the fraction of checks
    // that PASS. rate>0.99 means ≥99% of all checks must succeed.
    // (checks_failed is not a k6 built-in — use the 'checks' pass-rate instead)
    'checks': [
...
// registers/logs in 100 customer users.
export function setup() {
  ...
  console.log('🚀 Setting up concurrency benchmark environment...');
  ...
  // 2. Create a Test Event with exactly 50 seats (5 rows * 10 seats per row)
  const eventPayload = JSON.stringify({
    title: `k6 Concurrency Clash ${timestamp}`,
    description: 'High concurrency simulation event with limited seat availability.',
    category: 'MUSIC',
    venueId: venue.id,
  ...
  // 4. Fetch the generated Seat Map to get the Seat UUIDs
  ...
  // Flatten generated seats into a simple list of IDs
  const seatIds = [];
  seatMap.sections.forEach(section => {
    section.rows.forEach(row => {
      row.seats.forEach(seat => {
        seatIds.push(seat.seatId); // Correctly matches SeatResponse.seatId
      });
    });
  });
  console.log(`✅ Retrieved ${seatIds.length} available seats from the event seat map.`);

  // 5. Register and Login 100 customer users dynamically in a paced loop
  console.log('👥 Registering and authenticating 100 customer accounts in a paced loop...');
  ...
  console.log(`✅ Successfully authenticated ${customerTokens.length}/100 simulated customers.`);

  // Return all seeded assets to the default test workers
  return {
    eventId: event.id,
    seatIds: seatIds, // Array of 50 seat UUIDs
    customerTokens: customerTokens, // Array of 100 JWT tokens
    timestamp: timestamp
  };
}

// ── VIRTUAL USER STAGE ──────────────────────────────────────────────────────
// Executes in parallel. 100 VUs trigger checkouts concurrently.
export default function (data) {
  ...
  // We have 100 users and 50 seats.
  // VU 1-50 will attempt to book seats 1-50.
  // VU 51-100 will attempt to book the exact same seats 1-50 (generating the lock race condition).
  const seatIndex = (tokenIndex) % 50;
  const seatToBook = data.seatIds[seatIndex];

  const bookingPayload = JSON.stringify({
    eventId: data.eventId,
    seatIds: [seatToBook],
    paymentMethod: 'CARD',
    paymentToken: `mock_payment_token_${data.timestamp}_${__VU}` // Triggers payment-service mock success bypass with unique token per VU
  });
  ...
  // Introduce a slight micro-jitter (under 10ms) to ensure absolute multi-threaded alignment
